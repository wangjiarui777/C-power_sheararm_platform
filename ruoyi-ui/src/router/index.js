 ...
 * alwaysShow: true                 // 当你一个路由下面的 children 声明的路由大于1个时，自动会变成嵌套的模式--如引导页面
 * redirect: noRedirect             // 当设置 noRedirect 的时候路由在面包屑导航中不可被点击
 * name:'router-name'               // 设定路由的名字，一定要填写不然使用<keep-alive>时会出现各种问题
 * query: '{"id": 1, "name": "ry"}' // 访问路由的默认传递参数
 * roles: ['admin', 'common']       // 路由的角色权限
 * permissions: ['a:a:a', 'b:b:b']  // 路由的菜单权限
 * meta : {
 *   noCache: true                  // 如果设置为true，则不会被 <keep-alive> 缓存(默认 false)
 *   title: 'title'                 // 设置该路由在侧边栏和面包屑中展示的名字
 *   icon: 'svg-name'               // 设置该路由的图标，对应路径src/assets/icons/svg
 *   breadcrumb: false              // 设置为false不会在breadcrumb中显示
 *   activeMenu: '/system/user'     // 路由激活对应菜单
 * }
 */
