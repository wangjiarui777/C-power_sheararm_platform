...
- Windows 离线部署已有 Nginx、WinSW、Prometheus、备份和签名离线包脚本。
...
5. Java 对推理服务的强依赖会导致推理故障时整个平台无法启动；WinSW 启动依赖必须移除，改由健康状态表达降级。
...
开发环境允许保留统一 `phm-inference:5000`，但生产配置、WinSW 文件和监控必须使用双模型进程。

### 4.3 服务启动关系

- `phm-platform` 不依赖 `phm-infer-gear` 或 `phm-infer-bearing` 的 WinSW `<depend>`；推理服务先启动是优化，不是平台启动条件。
...
- 模型目录只读，版本目录不可变；日志、备份和离线包不得包含秘密、原始信号数组或完整内部令牌。
...
| 故障 | 处理 | 预期行为 |
|---|---|---|
| gear 进程停止 | 重启对应 WinSW 服务，检查模型哈希和内存 | 帧继续保存，gear 任务在截止时间后过期，bearing 正常 |
...
- 更新 `deployment/winsw/`、Nginx、防火墙说明、Prometheus 抓取和告警规则。
- 离线包包含双模型服务定义、固定 Python 运行时、模型清单、SHA-256、SBOM、备份/恢复脚本和签名验证。
...
1. 本文档及与实际端口、服务名、队列名一致的 `deployment/README.md` 更新。
2. 实时策略迁移、任务/结果字段迁移、策略 API、窗口内核和双模型调度器。
3. Python 批量推理接口、双模型运行入口、健康/指标接口和对应测试。
4. WinSW 双模型服务、Prometheus 抓取/告警、AOF/备份/恢复和离线包校验更新。
